...
```{r echo = TRUE}
packages = c('tidyverse','plotly','psych','ggridges','patchwork')
...
```
With this, we are now able to recieve more microdata regarding the different sub-groups in the chart as we hover our cursor over them.
...
Hence the next chart includes the 'facet_wrap()' parameter into the code chunk, which splits out the output into separate mini-charts grouped by the Education Level data.
...
Up next, a boxplot was plotted with geom_boxplot() and geom_point(), with facet_wrap() as below.
